...
createForm.addEventListener("submit", async (e) => {
  ...
  const to = document.getElementById("capsuleTo").value.trim();
  const occasion = document.getElementById("capsuleOccasion").value;
  ...
  try {
    ...
    // Document public : date + destinataire/occasion (toujours lisible,
    // sert au compte à rebours et à l'affichage avant ouverture)
    ...
    // Génère le lien, le QR code, et passe à l'écran de confirmation
...
async function loadCapsuleFromUrl() {
  ...
  try {
    ...
    const unlockAt = capsuleData.unlockAt.toDate();
    loadingEl.hidden = true;

    // Destinataire + occasion, affichés dans les deux états (verrouillé/révélé)
    const occasionLabel = capsuleData.occasion && capsuleData.occasion !== "Message" ? capsuleData.occasion : null;
    if (occasionLabel) {
      document.getElementById("lockedOccasion").textContent = occasionLabel;
      document.getElementById("lockedOccasion").hidden = false;
      document.getElementById("revealedOccasion").textContent = occasionLabel;
      document.getElementById("revealedOccasion").hidden = false;
    }
    document.getElementById("lockedTo").textContent = capsuleData.to
      ? `Pour ${capsuleData.to}`
      : "Capsule scellée";
